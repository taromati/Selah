...
public class TelegramInteractionRouter {
    ...
    public void processCallbackQuery(String messageId, ActionEvent actionEvent,
        ...
        log.debug("[TelegramInteractionRouter] Callback: actionId={}, handlers={}", actionEvent.actionId(), handlers.size());

        // 1. 임시 핸들 매칭
        var handle = temporaryHandles.remove(messageId);
        if (handle != null) {
            log.debug("[TelegramInteractionRouter] Matched temporary handle for messageId={}", messageId);
        ...
        for (InteractionHandler handler : handlers) {
            if (actionEvent.actionId().startsWith(handler.getActionIdPrefix())) {
                log.debug("[TelegramInteractionRouter] Matched handler: {} for actionId={}", handler.getClass().getSimpleName(), actionEvent.actionId());
