...
public class SetupWizard {
    ...
    public static void run() {
        ...
        if (Files.exists(configPath)) {
            ...
            config = ConfigGenerator.fromExistingConfig(configPath);
            hasExisting = config.hasAnyMessenger();
        } else {
        ...
        while (step < STEP_TITLES.length) {
            ...
            boolean next = switch (step) {
                case 0 -> MessengerStep.run(ui, config, hasExisting);
                case 1 -> {
        ...
        try {
            ...
            // memory-data 디렉토리 생성
            Files.createDirectories(Path.of("memory-data"));
