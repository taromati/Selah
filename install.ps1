...
if (Test-Path `$nssmExe) {
    try { & `$nssmExe stop selah-searxng 2>&1 | Out-Null } catch {}
    try { & `$nssmExe remove selah-searxng confirm 2>&1 | Out-Null; Write-Host "  [OK] SearXNG service removed" } catch {}
    try { & `$nssmExe stop selah 2>&1 | Out-Null } catch {}
    try { & `$nssmExe remove selah confirm 2>&1 | Out-Null; Write-Host "  [OK] Selah service removed" } catch {}
}
