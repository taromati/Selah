...
function Test-Java {
    ...
    if (Test-Path $bundledJava) {
        $ver = & $bundledJava -version 2>&1 | Select-Object -First 1
        if ($ver -match '"(\d+)') {
            $major = [int]$Matches[1]
            if ($major -ge 25) {
...
for %%f in ("%SELAH_HOME%\lib\selah-*.jar") do set JAR=%%f
if not defined JAR (
...
Write-Host ""
Write-Info "초기 설정을 시작합니다..."
Write-Info "제거하려면: powershell -File `"$SelahHome\uninstall.ps1`""
Write-Host ""

Set-Location $SelahHome
& (Join-Path $SelahHome "bin\selah.bat") setup
