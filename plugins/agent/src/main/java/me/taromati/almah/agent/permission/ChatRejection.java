...
public class ChatRejection {
    ...
    private String handleDenied(String toolName, String argumentsJson,
        ...
        if (approved == null) {
            // 중복 생성 방지: 동일 도구에 대한 WAITING_APPROVAL Task가 이미 있으면 스킵
            if (taskStoreService.existsWaitingApprovalForTool(toolName)) {
                return "이미 승인 대기 중인 동일 작업이 있습니다.";
            }
            // 타임아웃 → Task 생성 + WAITING_APPROVAL + 에스컬레이션 버튼
            ...
            // WAITING_APPROVAL 전환 + 버튼 메시지 전송 + messageId 저장
            escalationService.escalateForRoutine(task, toolName, argumentsJson);
