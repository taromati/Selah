...
public class SubagentToolHandler {
    ...
    @PostConstruct
    void register() {
        toolRegistry.register("spawn_subagent", DEFINITION, this::execute);
    }
     ...
     * visible tools에서 excludedTools를 제외하고, requestedTools가 있으면 교집합.
     */
    private List<String> resolveSubagentTools(List<String> requestedTools) {
        List<String> visible = new ArrayList<>(permissionGate.getVisibleTools(config.getSubagent().getExcludedTools()));

        if (requestedTools != null && !requestedTools.isEmpty()) {
            visible.retainAll(requestedTools);
        }
