...
public class RoutineOrchestrator {
    ...
    RoutineReport executeTask(AgentTaskItemEntity task, int remainingMinutes) {
        RoutineReport report = doExecuteTask(task, remainingMinutes);
        saveReportToSession(report);
        return report;
    }

    private RoutineReport doExecuteTask(AgentTaskItemEntity task, int remainingMinutes) {
        // IN_PROGRESS 전환
        ...
        try {
            ...
            if (result.roundsExhausted()) {
                // 예산 소진 → execution_log 기록 (기존 status 로직) + retryCount 증가
            ...
            // 자기 평가 파싱 (roundsExhausted=false인 경우에만 여기 도달)
            var selfEval = SelfEvalParser.parse(response);
            ...
            if (currentTask != null && !TaskStatus.isTerminal(currentTask.getStatus())
                ...
                switch (selfEval.status()) {
                    case PARTIAL -> {
                        // RV-2: 정체 감지
                        int consecutivePartials = countConsecutivePartials(task) + 1;
                        int maxPartials = config.getRoutine().getMaxConsecutivePartials();
                        recordExecutionLog(task, result, "PARTIAL");

                        if (consecutivePartials >= maxPartials) {
                            taskStoreService.incrementRetryCount(task.getId());
                            var updated = taskStoreService.findById(task.getId()).orElse(task);
                            if (updated.getRetryCount() >= task.getMaxRetries()) {
                                taskStoreService.transition(task.getId(), TaskStatus.FAILED);
                                triggerTaskRetrospect(task, "연속 PARTIAL 정체", false);
                                return new RoutineReport(task, response, false, toolsUsed, elapsedMs);
                            }
                        }
                        taskStoreService.transition(task.getId(), TaskStatus.PENDING);
                        log.info("[Routine] 자기 평가 PARTIAL (연속 {}/{}): {}", consecutivePartials, maxPartials, task.getTitle());
                        return new RoutineReport(task, response, false, toolsUsed, elapsedMs);
                    }
                    case FAIL -> {
                        // RV-3: retryCount 증가
                        ...
                        var updated = taskStoreService.findById(task.getId()).orElse(task);
                        if (updated.getRetryCount() >= task.getMaxRetries()) {
                            recordExecutionLog(task, result, "FAIL");
                            taskStoreService.transition(task.getId(), TaskStatus.FAILED);
                            String failReason = "자기 평가 FAIL" + (selfEval.reason() != null ? " (" + selfEval.reason() + ")" : "");
                            recordEpisode(task, failReason);
                            saveRoutineHistory(task, failReason, "FAILED");
                            triggerTaskRetrospect(task, failReason, false);
                            return new RoutineReport(task, response, false, toolsUsed, elapsedMs);
                        ...
                        recordExecutionLog(task, result, "FAIL");
                        taskStoreService.transition(task.getId(), TaskStatus.PENDING);
                        log.info("[Routine] 자기 평가 FAIL ({}/{}): {}", updated.getRetryCount(), task.getMaxRetries(), task.getTitle());
                        return new RoutineReport(task, response, false, toolsUsed, elapsedMs);
                    }
                    default -> {
                        // PASS, ABSENT: 기존 COMPLETED 동작 유지 (RV-1, RV-4)
                        recordExecutionLog(task, result, "COMPLETED");
                        taskStoreService.transition(task.getId(), TaskStatus.COMPLETED);
                        transitioned = true;
                    }
                }
            } else {
                // WAITING_APPROVAL 또는 이미 terminal — 기존 status 로직으로 기록
                recordExecutionLog(task, result);
            }
    ...
    private void recordExecutionLog(AgentTaskItemEntity task,
        ...
        recordExecutionLog(task, result, null);
    }
    ...
    private void recordExecutionLog(AgentTaskItemEntity task,
        ...
        try {
            ...
            // status 결정: override가 있으면 사용, 없으면 기존 로직
            String status;
    ...
    private List<String> resolveRoutineTools() {
        return permissionGate.getCoreTools(config.getRoutine().getExcludedTools());
    }
    ...
    private void saveReportToSession(RoutineReport report) {
        try {
            var session = sessionService.getOrCreateActiveSession(config.getChannelName());
            String sessionId = session != null ? session.getId() : null;
            String content = String.format("[루틴 보고] %s — %s\n%s",
                    report.task().getTitle(),
                    report.completed() ? "완료" : "미완료",
                    StringUtils.truncate(report.response(), 300));
            sessionService.saveMessage(sessionId, "assistant", content, null, null);
        } catch (Exception e) {
            log.warn("[Routine] 세션 기록 실패: {}", e.getMessage());
        }
    }
    ...
    /**
     * execution_log 끝에서 연속 PARTIAL 횟수를 센다.
     * 다른 status(FAIL, COMPLETED 등)가 나오면 체인이 끊긴다.
     */
    private int countConsecutivePartials(AgentTaskItemEntity task) {
        var entries = executionLogSerializer.deserialize(task.getExecutionLog());
        int count = 0;
        for (int i = entries.size() - 1; i >= 0; i--) {
            if ("PARTIAL".equals(entries.get(i).status())) {
                count++;
            } else {
                break;
            }
